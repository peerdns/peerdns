...
func (mp *MessagePool) AddMessage(msg *messages.ConsensusMessage) {
	mp.mutex.Lock()
...
func (mp *MessagePool) GetMessage(blockHash []byte) (*messages.ConsensusMessage, error) {
	mp.mutex.RLock()
...
func (mp *MessagePool) ListMessages() []*messages.ConsensusMessage {
	mp.mutex.RLock()
	defer mp.mutex.RUnlock()
