...
var (
	...
	ServiceType = runtime.RouterServiceType
...
func ServiceHandler(ctx context.Context, baseService *runtime.BaseService) (runtime.Service, error) {
	gLog := logger.G()
	gLog.Debug("Registering router service...")

	service, sErr := NewService(ctx, gLog, config.G(), baseService)
	if sErr != nil {
		return nil, errors.Wrap(sErr, "Failed to create new Router service")
	}

	if registered := runtime.Register(ServiceType, service); !registered {
		return nil, errors.Errorf("Failed to register Router service - already loaded")
	}
...
func init() {
	once.Do(func() {
		...
		if loaded := runtime.RegisterHandler(ServiceType, ServiceHandler); !loaded {
			panic("Router service could not be registered - already loaded")
		}
	})
}
