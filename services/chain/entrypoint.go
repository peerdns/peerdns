...
var (
	...
	ServiceType = runtime.ChainServiceType
...
func ServiceHandler(ctx context.Context, baseService *runtime.BaseService) (runtime.Service, error) {
	gLog := logger.G()
	gLog.Debug("Registering chain service...")

	service, sErr := NewService(ctx, gLog, config.G(), baseService)
	if sErr != nil {
		return nil, errors.Wrap(sErr, "Failed to create new chain service")
	}

	if registered := runtime.Register(ServiceType, service); !registered {
		return nil, errors.Errorf("Failed to register chain service")
	}
...
func init() {
	once.Do(func() {
		...
		if loaded := runtime.RegisterHandler(ServiceType, ServiceHandler); !loaded {
			panic("chain service could not be registered - most likely already exists")
		}
	})
}
